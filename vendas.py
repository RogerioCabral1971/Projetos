...
def vendas(url,dt_inicial, dt_fim, status):
  ...
  if len(lista)>0:
    ...
    df_prod=prod.produtos_vendidos(lista, dt_inicial, dt_fim)
...
def venda_geral(dt_inicial, dt_fim, status):
  ...
  df_vendas = st.session_state['notas_fiscais'].query(f'data>="{dt_inicial}" and data<="{dt_fim}"')
  ...
  pd.DataFrame(df_vendas).to_clipboard(excel=True)
  return df_vendas
